export default function ConfigLoading() {
  return (
    <div className="space-y-4">
      ...
      {/* Warning banner */}
      <div className="h-10 animate-pulse rounded-sm border border-[var(--surface-border)]/70 bg-[var(--surface-muted)]" />

      {/* General Settings section */}
      <section className="space-y-3 rounded-lg border border-[var(--surface-border)]/70 bg-[var(--surface-base)] p-4">
        <div className="mb-3 h-3 w-36 animate-pulse rounded bg-[var(--surface-muted)]" />
        <div className="grid gap-4 sm:grid-cols-2">
          {Array.from({ length: 5 }).map((_, idx) => (
            <div key={`general-${idx}`} className="space-y-2">
              <div className="h-3.5 w-36 animate-pulse rounded bg-[var(--surface-muted)]" />
      ...
      <section className="space-y-3 rounded-lg border border-[var(--surface-border)]/70 bg-[var(--surface-base)] p-4">
        <div className="mb-3 h-3 w-40 animate-pulse rounded bg-[var(--surface-muted)]" />
        <div className="grid gap-4 sm:grid-cols-2">
          {Array.from({ length: 5 }).map((_, idx) => (
            <div key={`retry-${idx}`} className="space-y-2">
              <div className="h-3.5 w-44 animate-pulse rounded bg-[var(--surface-muted)]" />
              <div className="h-3 w-52 animate-pulse rounded bg-[var(--surface-muted)]" />
              <div className="h-10 animate-pulse rounded-md bg-[var(--surface-muted)]" />
            </div>
          ))}
        </div>
      ...
      {/* Logging section */}
      <section className="space-y-3 rounded-lg border border-[var(--surface-border)]/70 bg-[var(--surface-base)] p-4">
        <div className="mb-3 h-3 w-20 animate-pulse rounded bg-[var(--surface-muted)]" />
        <div className="grid gap-4 sm:grid-cols-2">
